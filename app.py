...
st.title("⚾ MLB AI 감독 모드 V7.0 (투타 밸런스 시뮬레이터)")

@st.cache_data(ttl=3600)
def load_mlb_all_data():
    # 1. 타자 데이터 수집 (OPS 등 숫자로 변환)
...
# 💡 투타 밸런스 시뮬레이션 (팀 OPS가 높을수록 득점 확률 증가)
def run_simulation(home_era, away_era, home_ops, away_ops, num_sims=10000):
    ...
    # 메이저리그 평균 OPS를 약 0.720으로 가정하고 공격력 배율 계산
    home_attack_power = home_ops / 0.720 if home_ops > 0 else 1.0
    away_attack_power = away_ops / 0.720 if away_ops > 0 else 1.0

    # 득점 = 상대 투수 방어율 * 우리 팀 공격력 배율
    home_expected_runs = (a_era * home_attack_power) + 0.2 # 홈 어드밴티지 반영
...
try:
    ...
    st.success("✅ V7.0 시스템 준비 완료! (투타 밸런스 장착)")
    ...
    with tab1:
        ...
        if not df_schedule.empty:
            ...
            if st.button("🚀 10,000회 시뮬레이션 돌리기"):
                ...
                home_team, away_team = df_schedule.iloc[game_idx]['홈 팀'], df_schedule.iloc[game_idx]['어웨이 팀 (원정)']
                home_p, away_p = df_schedule.iloc[game_idx]['홈 선발투수'], df_schedule.iloc[game_idx]['어웨이 선발투수']
                
                # 각 팀의 주전 타자(타수 100 이상)들의 평균 OPS 계산
